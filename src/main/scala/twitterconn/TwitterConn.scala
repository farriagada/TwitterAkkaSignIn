...
object TwitterConn {
  ...
  def createSignatureUser(): String = {
    val rawURL: String = "GET&" + this.encodeUriComp("https://api.twitter.com/1.1/account/verify_credentials.json?include_email=true")+"&"
    val parameterString: String =
      "oauth_consumer_key=" + this.consumerKey +
        ...
        path("verifyCredentials"){
          get{
            parameters('token.as[String],'tokenSecret.as[String])((token, tokenSecret) => {
              ...
              val url = "https://api.twitter.com/1.1/account/verify_credentials.json?include_email=true"
              val authorization = headers.RawHeader("Authorization",
                """OAuth oauth_consumer_key="""" + this.consumerKey +
                  """", oauth_nonce="""" + this.third_nonce +
